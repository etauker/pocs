...
export class Periods {
    ...
    private formatPeriods(selection: d3.Selection<d3.EnterElement, PeriodInternal, d3.BaseType, any>) {
        const MAX_X_VALUE = this.component.getMaxValue();
        const instance = this;
        ...
        selection
            ...
            .style('fill', (data: PeriodInternal) => data.style && data.style.backgroundColour ? data.style.backgroundColour : this.DEFAULT_PERIOD_BACKGROUND_COLOR)
    ...
    private addBoundaries(periodComponents: d3.Selection<d3.EnterElement, PeriodInternal, d3.BaseType, any>) {
        const MAX_X_VALUE = this.component.getMaxValue();
        const instance = this;
        ...
        periodComponents
            ...
            .style('stroke', (data: PeriodInternal) => data.style && data.style.fillColour ? data.style.fillColour : this.DEFAULT_PERIOD_FILL_COLOR)
            .style('stroke-width', (data: PeriodInternal) => data.style && data.style.lineWidth ? data.style.lineWidth : this.DEFAULT_LINE_WIDTH)
    ...
    private addFill(periodComponents: d3.Selection<d3.EnterElement, PeriodInternal, d3.BaseType, any>) {
        const MAX_X_VALUE = this.component.getMaxValue();
        const instance = this;

        periodComponents
            .append('line')
            .classed('fill-line-solid', (data: PeriodInternal) => data.style && data.style.fillStyle  === 'line-solid')
            .classed('fill-line-dashed', (data: PeriodInternal) => data.style && data.style.fillStyle  === 'line-dashed')
            .classed('fill-none', (data: PeriodInternal) => data.style && data.style.fillStyle  === 'none')
            .attr('x1', (data: PeriodInternal) => (data.start % MAX_X_VALUE) / MAX_X_VALUE * instance.component.getWidth())
            ...
            .style('stroke', (data: PeriodInternal) => {
                if (data.style && data.style.fillStyle === 'none') {
                    return 0;
                }
                return data.style && data.style.fillColour ? data.style.fillColour : this.DEFAULT_PERIOD_FILL_COLOR;
            })
            .style('stroke-width', (data: PeriodInternal) => data.style && data.style.lineWidth ? data.style.lineWidth : this.DEFAULT_LINE_WIDTH)

    }

    private addAnnotations(periodComponents: d3.Selection<d3.EnterElement, PeriodInternal, d3.BaseType, any>) {
        const instance = this;
        const MAX_X_VALUE = this.component.getMaxValue();
        ...
        periodComponents
            ...
            .classed('text', true)
            .attr('x', (data: PeriodInternal) => ((data.start % MAX_X_VALUE)) / MAX_X_VALUE * instance.component.getWidth())
            .attr('y', (data: PeriodInternal) => (instance.component.scaleY(data.group) || 0) + (instance.component.scaleY.bandwidth() / 4 * 3) + (data.style && data.style.lineWidth ? data.style.lineWidth : this.DEFAULT_LINE_WIDTH))
            .attr('width', (data: PeriodInternal) => (data.end - data.start) / 2 * instance.component.getWidth())
            .attr('dx', (data: PeriodInternal) => (data.end - data.start) / MAX_X_VALUE * instance.component.getWidth() / 2)
