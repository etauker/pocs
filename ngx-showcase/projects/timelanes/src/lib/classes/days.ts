...
export class Days {
    ...
    constructor(parent: TimelanesGraphic, selection: d3.Selection<d3.BaseType, any, d3.BaseType, any>, data: PeriodInternal[], config: PeriodStyle = {}) {
        ...
        this.selection
            ...
            .style('fill', config && config.backgroundColour ? config.backgroundColour : 'white')
            .style('outline-color', config && config.borderColour ? config.borderColour : 'black')
            .style('outline-width', config && config.borderWidth ? config.borderWidth : '1px')
            .style('outline-style', config && config.borderStyle ? config.borderStyle : 'solid')

            if (config && config.fillStyle) {
                // TODO: implement
                // fillStyle
                // fillColour
                // lineWidth
    ...
    private static processInput(input: Period[]): PeriodInternal[] {
        const processed = input.map(period => {
